...
function getJiraIssues(){
  ...
  local response=$(curl -s -w "%{http_code}" -X GET -G "$jiraDomain/rest/api/2/search" \
    --data-urlencode "jql=updated > -1d AND (assignee=$jiraUserId OR \"QA Assignee 1[User Picker (single user)]\"=$jiraUserId OR \"Assignee 2[User Picker (single user)]\"=$jiraUserId)" \
...
while read -r credential; do
  ...
  jiraIssues=$(getJiraIssues $jiraDomain "$jiraAuthHeader" $jiraUserId)
